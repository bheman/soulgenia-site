...
export const metadata: Metadata = {
  title: "A super secretária que orquestra tudo [v3/orquestra]",
  description:
    "A Soul Genia transforma conversas, áudios, agenda e contas em briefing do dia, resumos e lembretes no seu WhatsApp. Você rege o dia e nenhuma mensagem sai sem a sua aprovação.",
  robots: {
    index: false,
    follow: false,
  },
...
export default function SoulGeniaHomePage() {
  const heroArtAlt =
    "Soul Genia, de terno branco em um ateliê claro, regendo com um gesto painéis flutuantes de WhatsApp, agenda do dia, lembretes, tarefas e resumo do dia";

  const {
    props: { srcSet: heroArtDesktopSrcSet },
  } = getImageProps({
    alt: heroArtAlt,
  ...
  const {
    props: { srcSet: heroArtMobileSrcSet, ...heroArtImgProps },
  } = getImageProps({
    alt: heroArtAlt,
  ...
  return (
    <main className="bg-background text-foreground">
      <LandingAnalytics page="soul_genia_v3_orquestra" />

      <section className="relative isolate overflow-hidden bg-[var(--v3-orquestra-bg)] text-[var(--v3-orquestra-ink)]">
        <div className="absolute inset-x-0 top-0 z-20">
          <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8">
            <a href="/" className="flex items-center gap-3" aria-label="Soul Genia">
              ...
              />
              <div>
                <p className="font-display text-lg leading-none text-[var(--v3-orquestra-ink)]">
                  Soul Genia
                </p>
                <p className="mt-1 text-xs text-[color-mix(in_srgb,var(--v3-orquestra-ink)_62%,transparent)] whitespace-nowrap">
            ...
            <nav
              className="hidden items-center gap-7 text-sm font-medium text-[color-mix(in_srgb,var(--v3-orquestra-ink)_78%,transparent)] lg:flex"
        ...
        <div className="relative h-[min(40svh,28rem)] w-full lg:absolute lg:inset-y-0 lg:left-0 lg:h-auto lg:w-[58%] lg:max-w-[1190px]">
          <picture>
            <source
          ...
          <div
            aria-hidden
            className="pointer-events-none absolute inset-0 bg-[linear-gradient(to_bottom,var(--v3-orquestra-bg)_0%,color-mix(in_srgb,var(--v3-orquestra-bg)_40%,transparent)_14%,transparent_30%,transparent_74%,var(--v3-orquestra-bg)_99%)] lg:bg-[linear-gradient(to_right,transparent_50%,var(--v3-orquestra-bg)_88%)]"
          />
          <div aria-hidden className="pointer-events-none absolute inset-x-0 top-0 hidden h-28 bg-[linear-gradient(to_bottom,color-mix(in_srgb,var(--v3-orquestra-bg)_90%,transparent)_0%,color-mix(in_srgb,var(--v3-orquestra-bg)_45%,transparent)_55%,transparent_100%)] lg:block" />
        </div>

        <div className="relative z-10 mx-auto grid w-full max-w-7xl px-5 pb-12 pt-7 sm:px-8 lg:min-h-[86svh] lg:grid-cols-[1.08fr_0.92fr] lg:items-center lg:gap-12 lg:pb-16 lg:pt-28">
          <div aria-hidden className="hidden lg:block" />

          <div className="hero-copy max-w-2xl lg:max-w-none">
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-[color-mix(in_srgb,var(--v3-orquestra-ink)_16%,transparent)] bg-white/64 px-4 py-2 text-sm font-semibold text-[color-mix(in_srgb,var(--v3-orquestra-ink)_84%,transparent)] backdrop-blur">
              <span className="aura-dot h-2 w-2 rounded-full bg-[var(--v3-orquestra-accent)]" />
              Onboarding assistido aberto
            </div>

            <h1 className="text-[clamp(2.6rem,10.5vw,4.5rem)] font-display leading-[0.95] tracking-tight text-[var(--v3-orquestra-ink)] md:text-[clamp(3rem,7vw,4.5rem)] lg:text-[clamp(3.1rem,4.6vw,4.5rem)]">
              A super secretária que orquestra tudo.
            </h1>

            <p className="mt-5 max-w-xl text-base leading-7 text-[color-mix(in_srgb,var(--v3-orquestra-ink)_78%,transparent)] sm:text-lg sm:leading-8">
              Conversas, áudios, agenda e contas deixam de disputar a sua
              atenção. A Soul Genia transforma tudo em briefing do dia, resumos
              e lembretes dentro do seu WhatsApp. Você rege o dia, ela cuida dos
              detalhes, e nenhuma mensagem sai sem a sua aprovação.
            </p>
            ...
            <div className="mt-7 flex flex-col gap-4 sm:flex-row">
              <TrackedCtaLink
                ...
                target={hasWhatsapp ? "_blank" : undefined}
                rel={hasWhatsapp ? "noreferrer" : undefined}
                className="motion-press shine-pass inline-flex min-h-14 items-center justify-center gap-2 rounded-full bg-primary px-8 py-4 text-base font-semibold text-white shadow-[0_16px_44px_-18px_rgba(5,64,72,0.55)] hover:bg-primary-light"
              >
              ...
              <a
                href="#beneficios"
                className="motion-press inline-flex min-h-14 items-center justify-center rounded-full border border-[color-mix(in_srgb,var(--v3-orquestra-ink)_24%,transparent)] bg-white/55 px-8 py-4 text-base font-semibold text-[var(--v3-orquestra-ink)] backdrop-blur hover:border-primary hover:text-primary"
              >
                Ver benefícios
              </a>
            </div>

            <ul className="mt-7 flex flex-wrap items-center gap-x-6 gap-y-2 text-sm font-medium text-[color-mix(in_srgb,var(--v3-orquestra-ink)_72%,transparent)]">
              {[
                "Briefing do dia no WhatsApp",
                "Grupos e áudios viram resumo",
                "Agenda, contas e lembretes organizados",
              ].map((point) => (
                <li key={point} className="inline-flex items-center gap-2">
                  <span
                    aria-hidden
                    className="h-1.5 w-1.5 rounded-full bg-[var(--v3-orquestra-accent)]"
                  />
                  {point}
                </li>
              ))}
