...
/* Hero v3/deverdade (arte H18) — art direction com getImageProps + <picture>:
   um único asset é baixado por breakpoint. Blend: borda direita da arte é
   ~var(--v3-deverdade-junction); gradiente CSS fecha a emenda com o creme. */
const heroProofPoints = [
  "Bom dia sem abrir cinco apps",
  "Lembra contas, remédios e retornos",
  "Aprende sua rotina com você",
];

const heroArtAlt =
  "Secretária da Soul Genia em um ateliê com luz de dia, de blazer verde-sage, organizando sobre a mesa painéis translúcidos com agenda, mensagens e lembretes";

const {
  props: { srcSet: heroArtDesktopSrcSet, sizes: heroArtDesktopSizes },
} = getImageProps({
  alt: heroArtAlt,
  src: "/images/heroes/v3/deverdade-desktop.webp",
  width: 1030,
  height: 941,
  priority: true,
...
const { props: heroArtMobileImgProps } = getImageProps({
  alt: heroArtAlt,
  src: "/images/heroes/v3/deverdade-mobile.webp",
  width: 840,
  height: 941,
  priority: true,
  sizes: "100vw",
...
export const metadata: Metadata = {
  title: "Uma secretária de verdade, sempre disponível [v3/deverdade]",
  description:
    "Uma secretária de verdade dentro do WhatsApp que você já usa: briefing do dia, lembretes de contas e retornos, pendências claras e aprovação antes de qualquer envio.",
  robots: {
    index: false,
    follow: false,
  },
...
export default function SoulGeniaHomePage() {
  return (
    <main className="bg-background text-foreground">
      <LandingAnalytics page="soul_genia_v3_deverdade" />

      <section className="relative overflow-hidden bg-[var(--v3-deverdade-bg)] text-[var(--ink)]">
          ...
          <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8">
            <a href="/" className="flex items-center gap-3" aria-label="Soul Genia">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-[color-mix(in_srgb,var(--ink)_62%,transparent)] whitespace-nowrap">
            ...
            <nav
              className="hidden items-center gap-7 text-sm font-medium text-[color-mix(in_srgb,var(--ink)_76%,transparent)] lg:flex"
            ...
            <TrackedCtaLink
              ...
              className="motion-press shine-pass hidden min-h-11 items-center rounded-full bg-primary px-5 py-2.5 text-sm font-semibold text-white shadow-[0_10px_40px_-12px_rgba(5,64,72,0.5)] hover:bg-primary-light md:inline-flex shrink-0 whitespace-nowrap"
            >
        ...
        <div className="relative flex flex-col lg:grid lg:min-h-[min(88svh,880px)] lg:grid-cols-[11fr_9fr]">
          <div className="relative h-[32svh] min-h-[230px] w-full lg:h-auto lg:min-h-0">
            <picture>
              <source
                media="(min-width: 1024px)"
                srcSet={heroArtDesktopSrcSet}
                sizes={heroArtDesktopSizes}
              />
              <img
                {...heroArtMobileImgProps}
                alt={heroArtAlt}
                fetchPriority="high"
                className="absolute inset-0 h-full w-full object-cover object-[center_27%] lg:object-right"
              />
            </picture>
            <div
              aria-hidden="true"
              className="absolute inset-x-0 top-0 h-32 bg-[linear-gradient(to_bottom,color-mix(in_srgb,var(--v3-deverdade-bg)_92%,transparent)_0%,color-mix(in_srgb,var(--v3-deverdade-bg)_50%,transparent)_45%,transparent_100%)]"
            />
            <div
              aria-hidden="true"
              className="absolute inset-x-0 bottom-0 h-28 bg-[linear-gradient(to_bottom,transparent,var(--v3-deverdade-bg))] lg:hidden"
            />
            <div
              aria-hidden="true"
              className="absolute inset-y-0 right-0 hidden w-[22%] bg-[linear-gradient(to_right,transparent,var(--v3-deverdade-junction))] lg:block"
            />
            <div
              aria-hidden="true"
              className="pointer-events-none absolute inset-y-0 left-0 hidden w-[9%] bg-[linear-gradient(to_right,var(--v3-deverdade-junction),transparent)] lg:block"
            />
          </div>

          <div className="relative lg:flex lg:items-center lg:bg-[linear-gradient(to_right,var(--v3-deverdade-junction),var(--v3-deverdade-bg)_42%)]">
            <div className="hero-copy mx-auto w-full max-w-2xl px-5 pb-16 pt-8 sm:px-8 lg:max-w-xl lg:px-10 lg:py-32 xl:max-w-2xl xl:px-14">
              <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-[color-mix(in_srgb,var(--ink)_16%,transparent)] bg-white/50 px-4 py-2 text-sm font-semibold text-[var(--ink)] backdrop-blur">
                <span className="aura-dot h-2 w-2 rounded-full bg-[var(--v3-deverdade-brass)]" />
                Sem instalar outro app
              </div>

              <h1 className="max-w-4xl font-display text-[clamp(2.35rem,9.4vw,4.5rem)] leading-[0.98] tracking-tight text-[var(--ink)] lg:text-[clamp(2.9rem,4.5vw,4.5rem)]">
                Como ter uma secretária{" "}
                <span className="whitespace-nowrap underline decoration-[var(--v3-deverdade-sage)] decoration-4 underline-offset-8">
                  de verdade
                </span>
                , sempre disponível.
              </h1>

              <p className="mt-5 max-w-2xl text-base leading-7 text-[color-mix(in_srgb,var(--ink)_80%,transparent)] sm:text-lg sm:leading-8">
                Ela organiza mensagens, áudios, agenda e contas no WhatsApp que
                você já usa, do jeito que uma boa secretária faria. O dia começa
                com um briefing e termina com as pendências claras. E antes de
                qualquer envio, ela pergunta primeiro.
              </p>
              ...
              <div className="mt-7 flex flex-col gap-4 sm:flex-row">
                <TrackedCtaLink
                  ...
                  target={hasWhatsapp ? "_blank" : undefined}
                  rel={hasWhatsapp ? "noreferrer" : undefined}
                  className="motion-press shine-pass inline-flex min-h-14 items-center justify-center gap-2 rounded-full bg-primary px-8 py-4 text-base font-semibold text-white shadow-[0_14px_44px_-16px_rgba(5,64,72,0.55)] hover:bg-primary-light"
                >
                ...
                <a
                  href="#beneficios"
                  className="motion-press inline-flex min-h-14 items-center justify-center rounded-full border border-[color-mix(in_srgb,var(--v3-deverdade-sage)_55%,var(--ink))] bg-white/40 px-8 py-4 text-base font-semibold text-[var(--ink)] backdrop-blur hover:bg-white/70"
                >
                  Ver benefícios
                </a>
              </div>

              <ul className="mt-9 flex flex-wrap gap-x-6 gap-y-3 text-sm font-semibold text-[color-mix(in_srgb,var(--ink)_78%,transparent)]">
                {heroProofPoints.map((point) => (
                  <li key={point} className="flex items-center gap-2.5">
                    <span
                      aria-hidden="true"
                      className="h-1.5 w-1.5 shrink-0 rounded-full bg-[color-mix(in_srgb,var(--v3-deverdade-sage)_65%,var(--ink))]"
                    />
                    {point}
                  </li>
                ))}
        ...
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-x-0 bottom-0 z-10 hidden h-20 bg-[linear-gradient(to_bottom,transparent,var(--background))] lg:block"
        />
      </section>
