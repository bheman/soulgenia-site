...
export const metadata: Metadata = {
  title: "GatewayPort checkout return pending",
  robots: {
    index: false,
    follow: false,
  },
...
export default function GatewayPortTestCheckoutSuccessPage() {
  const resolution = resolveTestCheckoutProviderReturn("success");
  ...
  return (
    <main className="min-h-screen bg-[var(--ink)] px-5 py-8 text-[var(--cream)] sm:px-8">
      <section className="mx-auto flex max-w-3xl flex-col gap-6 rounded-lg border border-white/15 bg-white/[0.04] p-6 sm:p-8">
        <p className="text-xs font-bold uppercase tracking-[0.22em] text-[var(--brass)]">Browser success return</p>
        <h1 className="text-4xl leading-tight text-[var(--cream)]">Payment confirmation is still pending provider verification.</h1>
        <p className="text-base leading-7 text-[color-mix(in_srgb,var(--cream)_76%,transparent)]">
          A success URL only proves the browser came back from a provider-style flow. GatewayPort does not grant an
          entitlement, WhatsApp connection, token, API access, media processing, or scheduled actions from this route.
        </p>
